...
enum  pid = ctRegex!`PeopleIDs\[(\d+)\] = (\d+);`
	...
	// , hexa = ctRegex!`// hexAvailability: ([0-9a-f]+)[\n\r]+AvailableAtSlot\[\d+\].push\((\d+)\);` // old version
	, aas = ctRegex!(`AvailableAtSlot\[(\d+)\]\.push\((\d+)\)`) // new version
...
BigInt[int] availabilityOf(string contents) {
	import std.conv;
	BigInt[int] ans;
	version(none) { // old version
		foreach(m; contents.matchAll(hexa)) {
			ans[to!int(m[2])] = BigInt("0x"~m[1]);
		}
	} else { // new version
		int slots = 0;
		foreach(m; contents.matchAll(aas)) {
			auto tmp = m[1].to!int;
			if (tmp > slots) slots = tmp;
		}
		foreach(m; contents.matchAll(aas)) {
			auto person = to!int(m[2]);
			if (person !in ans) ans[person] = BigInt(0);
			ans[person] |= BigInt(1) << (slots - m[1].to!int);
		}
	}
	return ans;
}
...
void main(string[] args) {
	...
	foreach(slots; 1..5) {
		...
		foreach(opt; nBits(cast(int)(m.uintLength*32), slots, 3)) {
			...
			int missed = 0;
			foreach(k,v; avail) {
				//if (k == "Rohit" || k == "Andrew") continue; // duplicate or all-false
				if ((v&opt) == 0) missed += 1;
