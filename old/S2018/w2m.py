...
# hexa = re.compile(r"// hexAvailability: ([0-9a-f]+)[\n\r]+AvailableAtSlot\[\d+\].push\((\d+)\);") # no longer working; site changed format
aas = re.compile(r'AvailableAtSlot\[(\d+)\]\.push\((\d+)\)') # new version
...
indexOfMask = {}
bits = max(int(m.group(1)) for m in aas.finditer(s))
for m in aas.finditer(s):
    time, person =  int(m.group(1)), int(m.group(2))
    if person not in indexOfMask:
        indexOfMask[person] = len(masks)
        masks.append(0)
    masks[indexOfMask[person]] |= 1 << (bits-time)
for pid,idx in indexOfMask.items():
    num = masks[idx]
    openness[namesById[str(pid)]] = setBits(num), setBits(num^(num>>1))
