...
target="$HOME/public_html/2501/S2016/"

htmlheader='﻿<!DOCTYPE html>
<html>
<head>
	<title>CS 2501: Teaching Assistant Practicum</title>
...
function update() {
	...
	echo "$htmlheader"
	getheader "$f" | sed -e 's/<h1>/<h1 class="top">CS 2501: /'
	echo '<div class="menu">'
	last="!";
	for g in *.md
	do
...
#	echo '<blockquote style="background-color:#fbb; font-size:150%">This page does not represent <a href="../">the most current semester</a> of this course; it is present merely as an archive.</blockquote>';
	pandoc "$f" --smart --from=markdown_mmd | tail -n +2 | sed -e 's!p>&lt;\(form .*\)&gt;</p!\1!g' \
		-e 's!\(<.*\)&quot;\(.*\)&quot;\(.*>\)!\1"\2"\3!g' \
		-e 's!\(<.*\)&quot;\(.*\)&quot;\(.*>\)!\1"\2"\3!g' \
		-e 's!\(<.*\)&quot;\(.*\)&quot;\(.*>\)!\1"\2"\3!g' \
		-e 's!\(<.*\)&quot;\(.*\)&quot;\(.*>\)!\1"\2"\3!g'
...
for f in *.{png,svg,jpg,tar,txt,c,h,s,ys,yo,pdf,gnuplot,hcl,zip,d}
do
	if [ "$f" -nt "$target$f" ]
	then
		echo 'copying' "$f"
